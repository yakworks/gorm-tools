...
> :memo: GET requests on collection resources should provide a sufficient filter mechanism as well
...
> :memo: It is no option to encode the lengthy structured request information in header parameters.
...
> :memo: Resource IDs with respect to PUT requests are maintained by the client and passed as a
...
> :memo: POST should be used for scenarios that cannot be covered by the other methods
...
> :bulb:  **Resource IDs** with respect to POST requests are created and maintained by server and
...
> :bulb: since implementing PATCH correctly is a bit tricky, we strongly suggest to choose one and
...
> :bulb: **while a _batch_** defines a collection of requests triggering independent processes, a *bulk* defines a collection of independent resources created or updated together in one request. With respect to response processing this distinction normally does not matter.
