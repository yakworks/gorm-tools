...
class OrgSpec extends Specification {
    ...
    def "Check list"() {
        expect:
        Org.dao.query().size() == 10
    }

    def "Filter by Name eq"() {
        when:
        List list = Org.dao.query([criteria: [name: "Org#23"], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#23"
    }
    ...
    def "Filter by id eq"() {
        when:
        List list = Org.dao.query([criteria: [id: 24], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#23"
    }
    ...
    def "Filter by id inList"() {
        when:
        List list = Org.dao.query([criteria: [id: [24, 25]], max: 150])
        then:
        list.size() == 2
        list[0].name == "Org#23"
    }
    ...
    def "Filter by Name ilike"() {
        when: "eq"
        List list = Org.dao.query([criteria: [name: "Org#2%"], max: 150])
        then:
        list.size() == 11
        list[0].name == "Org#2"
        list[1].name == "Org#20"
    ...
    def "Filter by nested id"() {
        when: "eq"
        List list = Org.dao.query([criteria: [address: [id: 2]], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#1"
        list[0].address.id == 2
    ...
    def "Filter by nested id inList"() {
        when:
        List list = Org.dao.query([criteria: [address: [id: [24, 25, 26]]], max: 150])
        then:
        list.size() == 3
        list[0].name == "Org#23"
    }
    ...
    def "Filter by nested string"() {
        when: "eq"
        List list = Org.dao.query([criteria: [address: [city: "City#2"]], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#2"
        list[0].address.id == 3
    ...
    def "Filter by boolean"() {
        when:
        List list = Org.dao.query([criteria: [isActive: true], max: 150])
        then:
    ...
    def "Filter by boolean in list"() {
        when:
        List list = Org.dao.query([criteria: [isActive: [false]], max: 150])
        then:
        list.size() == 50
    ...
    def "Filter by BigDecimal"() {
        when:
        List list = Org.dao.query([criteria: [revenue: 200.0], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#2"
    }
    ...
    def "Filter by BigDecimal in list"() {
        when:
        List list = Org.dao.query([criteria: [revenue: [200.0, 500.0]], max: 150])
        then:
        list.size() == 2
        list[0].name == "Org#2"
        list[1].name == "Org#5"
    ...
    def "Filter by Date"() {
        when:
        List list = Org.dao.query([criteria: [testDate: (new Date() + 1).clearTime()], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#1"
    }
    ...
    def "Filter by Date le"() {
        when:
        List list = Org.dao.query([criteria: ['testDate.$lte': (new Date() + 1).clearTime()], max: 150])
        then:
    ...
    def "Filter by xxxId 1"() {
        ...
        List list = Org.dao.query([criteria: [refId: 200], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#1"
    }
    ...
    def "Filter by xxxId 2"() {
        ...
        List list = Org.dao.query([criteria: ["address.testId": 9], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#3"
    }
    ...
    def "Filter by xxxId 3"() {
        ...
        List list = Org.dao.query([criteria: [address: [testId: 3]], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#1"
    }
    ...
    def "Filter by xxxId 4"() {
        when: "xxxId in nested domain"
        List list = Org.dao.query([criteria: ["address.testId": [9, 12]], max: 150])
        then:
        list.size() == 2
        list[0].name == "Org#3"
    }
    ...
    def "Filter by xxxId 4 criteria as JSON"() {
        ...
        List list = Org.dao.query(params)
        then:
        list.size() == 2
        list[0].name == "Org#3"
    }
    ...
    def "Filter with `or` "() {
        when:
        List list = Org.dao.query([criteria: ['$or': ["name": "Org#1", "address.id": 4]], max: 150])
        then:
        list.size() == 2
        list[0].name == "Org#1"
        list[1].name == "Org#3"
    ...
    def "Filter with `or` on low level"() {
        when:
        List list = Org.dao.query([criteria: [address: ['$or': ["city": "City#1", "id": 4]]], max: 150])
        then:
        list.size() == 2
        list[0].name == "Org#1"
        list[1].name == "Org#3"
    ...
    def "Filter with several `or` on one level"() {
        ...
        List list = Org.dao.query([criteria: ['$or': [["address.id": 5], ["name": "Org#1", "address.id": 4]]], max: 150])
        then:
    ...
    def "Filter with several `or` on one level2"() {
        when:
        List list = Org.dao.query([criteria: ["\$or": [["address.id": 5], ["address.id": 4]]], max: 150])
        then:
        list.size() == Org.createCriteria().list() {
            or {
                address {
    ...
    def "Filter with `or` with like"() {
        when:
        List list = Org.dao.query([criteria: ["\$or": ["name": "Org#2%", "address.id": 4]], max: 150]).sort { it.id }
        then:
        list.size() == 12
        list[0].name == "Org#2"
        list[1].name == "Org#3"
    ...
    def "Filter with `between()`"() {
        when:
        List list = Org.dao.query([criteria: [id: ["\$between": [2, 10]]], max: 150]).sort { it.id }
        then:
        list.size() == 9
        list[0].name == "Org#1"
        list[1].name == "Org#2"
    ...
    def "Filter with `in()`"() {
        when:
        List list = Org.dao.query([criteria: [id: ["\$in": [24, 25]]], max: 150]).sort { it.id }
        then:
        list.size() == 2
        list[0].name == "Org#23"
    }
    ...
    def "Filter with `inList()`"() {
        when:
        List list = Org.dao.query([criteria: [id: ["\$inList": [24, 25]]], max: 150]).sort { it.id }
        then:
        list.size() == 2
        list[0].name == "Org#23"
    }
    ...
    def "Filter by Name ilike()"() {
        when:
        List list = Org.dao.query([criteria: [name: ["\$ilike": "Org#2%"]], max: 150])
        then:
        list.size() == 11
        list[0].name == "Org#2"
        list[1].name == "Org#20"
    ...
    def "Filter with `gt()`"() {
        when:
        List list = Org.dao.query([criteria: [id: ["\$gt": 95]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `ge()`"() {
        when:
        List list = Org.dao.query([criteria: [id: ['\$gte': 95]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `ge()` for bigdecimal"() {
        when:
        List list = Org.dao.query([criteria: [revenue: ["\$gte": 9500.0]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `lte()` then another value"() {
        when:
        List list = Org.dao.query([criteria: [revenue: ["\$ltef": "credit"]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `gte()` then another value"() {
        when:
        List list = Org.dao.query([criteria: [revenue: ["\$gtef": "credit"]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `gt()` then another value"() {
        when:
        List list = Org.dao.query([criteria: [revenue: ["\$gtf": "credit"]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `lt()` then another value"() {
        when:
        List list = Org.dao.query([criteria: [revenue: ["\$ltf": "credit"]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `lt()`"() {
        when:
        List list = Org.dao.query([criteria: [id: ["\$lt": 5]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `le()`"() {
        when:
        List list = Org.dao.query([criteria: [id: ["\$lte": 5]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `isNull` object"() {
        when:
        List list = Org.dao.query([criteria: [credit: ["\$isNull": true]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `isNull` when val"() {
        when:
        List list = Org.dao.query([criteria: [credit: "\$isNull"], max: 150]).sort { it.id }
        then:
        list.size() == Org.createCriteria().list() { isNull "credit" }.size()
    }
    ...
    def "Filter with `isNull` when just 'null'"() {
        when:
        List list = Org.dao.query([criteria: [credit: null], max: 150]).sort { it.id }
        then:
        list.size() == Org.createCriteria().list() { isNull "credit" }.size()
    }
    ...
    def "Filter with `not in()`"() {
        when:
        List list = Org.dao.query([criteria: [id: ["\$nin": [2, 3, 4, 5]]], max: 150])
        then:
    ...
    def "test paging"() {
        when:
        List list = Org.dao.query([max: 20])
        then:
        list.size() == 20
    }
    ...
    def "test closure with params"() {
        when:
        List list = Org.dao.query([criteria: [id: ["\$in": [24, 25, 18, 19]]], max: 150]) {
            gt "id", 19L
        }
    ...
    def "test quick search"() {
        when:
        List list = Org.dao.query([criteria: ['$quickSearch': "Org#2%"], max: 150])
        then:
        list.size() == 11
