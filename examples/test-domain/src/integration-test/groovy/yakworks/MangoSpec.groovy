...
class MangoSpec extends Specification {
    ...
    def "Filter by id inList"() {
        when:
        List list = Org.query([criteria: [id: [24, 25]], max: 150])
        then:
        list.size() == 2
        list[0].name == "Org24"
    }
    ...
    def "Filter by Name ilike"() {
        when: "eq"
        List list = Org.query([criteria: [name: "Org2%"], max: 150])
        then:
        list.size() == 11
        list[0].name == "Org2"
        list[1].name == "Org20"
    ...
    def "Filter by nested id"() {
        when: "eq"
        List list = Org.repo.query([criteria: [location: [id: 2]]])
        then:
        list.size() == 1
        list[0].name == "Org2"
    ...
    def "Filter by nested id inList"() {
        when:
        List list = Org.repo.query([criteria: [location: [id: [24, 25, 26]]]])
        then:
        list.size() == 3
    ...
    def "Filter by nested string"() {
        when: "eq"
        List list = Org.query([criteria: [location: [city: "City2"]]])
        then:
        list.size() == 1
        list[0].name == "Org2"
        list[0].location.id == 2
    ...
    def "Filter by nested string ext"() {
        when: "eq"
        List list = Org.query([criteria: [ext: [text1: "Ext4"]]])
        then:
        list.size() == 1
        list[0].name == "Org4"
    ...
    def "Filter by nested string ilike"() {
        when: "eq"
        List list = Org.repo.query([criteria: [location: [city: "City2%"]], max: 50])
        then:
        list.size() == 11
    ...
    def "Filter by boolean"() {
        when:
        List list = Org.repo.query([criteria: [inactive: true], max: 100])
        then:
    ...
    def "Filter by boolean in list"() {
        when:
        List list = Org.repo.query([criteria: [inactive: [false]], max: 100])
        then:
        list.size() == 50
    ...
    def "Filter by BigDecimal"() {
        when:
        List list = Org.repo.query([criteria: [revenue: 2.50]])
        then:
        list.size() == 1
        list[0].revenue == 2.50
        list[0].name == "Org3"
    ...
    def "Filter by BigDecimal in list"() {
        when:
        List list = Org.query([criteria: [revenue: [2.50, 3.75]]])
        then:
        list.size() == 2
        list[0].revenue == 2.50
        list[0].name == "Org3"
    ...
    def "Filter by Date"() {
        when:
        List list = Org.repo.query([criteria: [actDate: (new Date() + 1).clearTime()]])
        then:
        list.size() == 1
        list[0].name == "Org1"
    }
    ...
    def "Filter by Date le"() {
        when:
        List list = Org.repo.query([criteria: ['actDate.$lte': (new Date() + 1).clearTime()], max: 150])
        then:
    ...
    def "Filter by xxxId 1"() {
        ...
        List list = Org.repo.query([criteria: [locationId: 2]])
        then:
        list.size() == 1
        list[0].locationId == 2
        list[0].location.id == 2
    ...
    def "Filter by ext.parentOrg.id"() {
        ...
        List list = Org.repo.query([criteria: ["ext.orgParentId": 1], max: 150])
        then:
        list.size() == 49
    }
    ...
    def "Filter by xxxId 3"() {
        ...
        List list = Org.repo.query([criteria: [ext: [orgParentId: 2]], max: 150])
        then:
        list.size() == 49
    }
    ...
    def "Filter by xxxId 4"() {
        when: "xxxId in nested domain"
        List list = Org.repo.query([criteria: ["ext.orgParentId": [1, 2]], max: 150])
        then:
    ...
    def "Filter with `or` "() {
        when:
        List list = Org.repo.query([criteria: ['$or': ["name": "Org1", "location.id": 4]]])
        then:
        list.size() == 2
        list[0].name == "Org1"
        list[1].name == "Org4"
    ...
    def "Filter with `or` on low level"() {
        when:
        List list = Org.repo.query([criteria: [location: ['$or': ["city": "City1", "id": 4]]], max: 150])
        then:
        list.size() == 2
        list[0].name == "Org1"
        list[1].name == "Org4"
    ...
    def "Filter with several `or` on one level"() {
        when:
        List list = Org.repo.query([criteria: ['$or': [
            ["location.id": 5],
    ...
    def "Filter with several `or` on one level2"() {
        when:
        List list = Org.repo.query([criteria: ['$or': [["location.id": 5], ["location.id": 4]]]])
        then:
        list.size() == 2
    }
    ...
    def "Filter with `or` with like"() {
        when:
        List list = Org.repo.query([criteria: ['$or': ["name": "Org2%", "location.id": 4]], max: 50])
        then:
        list.size() == 12
    }
    ...
    def "Filter with `between()`"() {
        when:
        List list = Org.repo.query([criteria: [id: ['$between': [2, 10]]]])
        then:
        list.size() == 9
    }
    ...
    def "Filter with `in()`"() {
        when:
        List list = Org.repo.query([criteria: [id: ["\$in": [24, 25]]]])
        then:
        list.size() == 2
    }
    ...
    def "Filter with `inList()`"() {
        when:
        List list = Org.repo.query([criteria: [id: ["\$inList": [24, 25]]]])
        then:
        list.size() == 2
    }
    ...
    def "Filter by Name ilike()"() {
        when:
        List list = Org.repo.query([criteria: [name: ['$ilike': "Org2%"]], max: 50])
        then:
        list.size() == 11
    }
    ...
    def "Filter with `gt()`"() {
        when:
        List list = Org.repo.query([criteria: [id: ['$gt': 95]]])
        then:
        list.size() == 5
    }
    ...
    def "Filter with `ge()`"() {
        when:
        List list = Org.repo.query([criteria: [id: ['$gte': 95]]])
        then:
        list.size() == 6
    }
    ...
    def "Filter with `ge()` for bigdecimal"() {
        when:
        List list = Org.repo.query([criteria: [revenue: ['$gte': 100.0]], max: 150])
        then:
    ...
    def "Filter with `lte()` then another value"() {
        when:
        List list = Org.repo.query([criteria: [revenue: ['$ltef': "creditLimit"]], max: 150])
        then:
    ...
    def "Filter with `gte()` then another value"() {
        when:
        List list = Org.repo.query([criteria: [revenue: ["\$gtef": "creditLimit"]], max: 150])
        then:
    ...
    def "Filter with `gt()` then another value"() {
        when:
        List list = Org.repo.query([criteria: [revenue: ["\$gtf": "creditLimit"]], max: 150])
        then:
    ...
    def "Filter with `lt()` then another value"() {
        when:
        List list = Org.repo.query([criteria: [revenue: ["\$ltf": "creditLimit"]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `lt()`"() {
        when:
        List list = Org.repo.query([criteria: [id: ["\$lt": 5]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `le()`"() {
        when:
        List list = Org.repo.query([criteria: [id: ["\$lte": 5]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `isNull` object"() {
        when:
        List list = Org.repo.query([criteria: [name2: ["\$isNull": true]], max: 150]).sort { it.id }
        then:
    ...
    def "Filter with `isNull` when val"() {
        when:
        List list = Org.repo.query([criteria: [name2: '$isNull'], max: 100]).sort { it.id }
        then:
        list.size() == 50
    }
    ...
    def "Filter with `isNull` when just null"() {
        when:
        List list = Org.repo.query([criteria: [name2: null], max: 100])
        then:
        list.size() == Org.createCriteria().list() { isNull "name2" }.size()
    }
    ...
    def "Filter with `not in()`"() {
        when:
        List list = Org.repo.query([criteria: [id: ["\$nin": [2, 3, 4, 5]]], max: 150])
        then:
    ...
    def "test closure with params"() {
        when:
        List list = Org.repo.query([criteria: [id: ['$in': [24, 25, 18, 19]]], max: 150]) {
            gt "id", 19L
        }
    ...
    def "test quick search"() {
        when:
        List list = Org.repo.query([criteria: ['$q': "Org2"], max: 150])
        then:
        list.size() == 11
