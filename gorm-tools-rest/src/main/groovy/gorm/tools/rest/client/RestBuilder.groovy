...
class RestBuilder {
    ...
    protected void registerMessageConverters(RestTemplate restTemplate) {
        ...
        final stringConverter = messageConverters.find { HttpMessageConverter httpMessageConverter ->
        ...
        if(ClassUtils.isPresent("org.springframework.http.converter.json.MappingJackson2HttpMessageConverter", getClass().getClassLoader())) {
            messageConverters.removeAll { HttpMessageConverter httpMessageConverter ->
                httpMessageConverter.getClass().name == "org.springframework.http.converter.json.MappingJackson2HttpMessageConverter"
            }
        }
        if(ClassUtils.isPresent("org.springframework.http.converter.json.MappingJacksonHttpMessageConverter", getClass().getClassLoader())) {
            messageConverters.removeAll { HttpMessageConverter httpMessageConverter ->
                httpMessageConverter.getClass().name == "org.springframework.http.converter.json.MappingJacksonHttpMessageConverter"
            }
        }
        if(ClassUtils.isPresent("org.springframework.http.converter.json.GsonHttpMessageConverter", getClass().getClassLoader())) {
            messageConverters.removeAll { HttpMessageConverter httpMessageConverter ->
                httpMessageConverter.getClass().name == "org.springframework.http.converter.json.GsonHttpMessageConverter"
            }
        }
