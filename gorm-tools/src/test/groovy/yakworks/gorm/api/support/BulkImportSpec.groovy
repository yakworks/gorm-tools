...
class BulkImportSpec extends Specification implements GormHibernateTest {
    ...
    BulkImportSupport getBulkImportSupport(){
        def bis = new BulkImportSupport(KitchenSink)
        AppCtx.autowire(bis)
        return bis
    }
    ...
    void "simple bulk insert"() {
        ...
        Long jobId = bulkImportSupport.bulk(list, setupSyncJobArgs())
    ...
    void "test bulk update"() {
        ...
        jobId = bulkImportSupport.bulk(list, setupSyncJobArgs(DataOp.update))
    ...
    void "success bulk insert with csv using usePathKeyMap"() {
        ...
        Long jobId = bulkImportSupport.bulk(data, args)
        def job = TestSyncJob.get(jobId)
...
//        payload[1].name == "Sink2"
    ...
    void "test buildErrorMap"() {
        ...
        bulkImportSupport.buildErrorMap(data, null) == data

        and: "when includes provided"
        bulkImportSupport.buildErrorMap(data, ["id", "num"]) == [id:1, num: "cust-1"]
    }
    ...
    void "test buildSuccessMap"() {
        ...
        Map result = bulkImportSupport.buildSuccessMap(kitchenSink, ["name", "ext.name"])
    ...
    void "test empty data"() {
        when:
        Long jobId = bulkImportSupport.bulk(null, setupSyncJobArgs())
        ...
        when:
        jobId = bulkImportSupport.bulk([], setupSyncJobArgs())
