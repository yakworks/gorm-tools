...
trait BuildEntityTester<D> extends GormToolsDataTester {
    ...
    Class<D> getEntityClass() {
        if (!entityClass)
            this.entityClass = (Class<D>) GenericTypeResolver.resolveTypeArgument(getClass(), BuildEntityTester.class)

        return entityClass
    }

    //Call back out to the methods added with the TestDataBuilder
    D build() {
