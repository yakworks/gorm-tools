...
class OrgSpec extends Specification {
    ...
    def "Filter by nestedId"() {
        when: "eq"
        List list = Org.dao.list([criteria:[addressId: 2], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#1"
        list[0].address.id == 2
    ...
    def "Filter by BigDecimal"() {
        when:
        List list = Org.dao.list([criteria:[revenue: "200"], max: 150])
        then:
        list.size() == 1
        list[0].name == "Org#2"
    }
    ...
    def "Filter by BigDecimal in list"() {
        when:
        List list = Org.dao.list([criteria:[revenue: ["200", "500"]], max: 150])
        then:
        list.size() == 2
        list[0].name == "Org#2"
        list[1].name == "Org#5"
    ...
    def "Filter with `ge()` for bigdecimal"(){
        when:
        List list = Org.dao.list([criteria:[revenue: ["\$gte": "9500"]], max: 150]).sort{it.id}
        then:
