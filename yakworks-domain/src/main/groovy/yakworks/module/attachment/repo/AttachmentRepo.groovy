...
*/
package yakworks.module.attachment.repo
...
@GormRepository
@CompileStatic
class AttachmentRepo implements GormRepo<Attachment> {
    ...
    void beforeBind(Attachment attachment, Map p, BeforeBindEvent ev) {
        ...
        if (ev.isBindCreate()) {
            ...
            if (!p['id']) p['id'] = idGenerator.getNextId('Attachment.id')
     ...
     */
    @Transactional
    Results copy(Attachment source) {
        if(source == null) return null
        InputStream inputStream = source.inputStream as InputStream
        if (!inputStream) return Results.error(source.id, "failed")
        byte[] data = IOUtils.toByteArray(inputStream)
        if (data.length == 0) {
            log.warn("Empty attachment found  id:$source.id")
            return Results.error(source.id, "failed")
        }
        ...
        return new Results(id:copy.id, entity: copy, ok:true, code:"finished.ok")
    }
    ...
    String getDownloadUrl(Attachment attachment) {
        linkGenerator.link(uri: "/attachment/download/${attachment.id}")
    }
